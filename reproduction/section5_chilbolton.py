...
def plot_figure7(data1: dict, data2: dict):
    ...
    ax.set_aspect("equal")
    fig.tight_layout()
    out = OUT_DIR / "fig7_chilbolton_layout.png"
...
def plot_figure8(results1: list, results2: list, data1: dict, data2: dict):
    ...
    for col, (results, data) in enumerate([(results1, data1), (results2, data2)]):
        ...
        for row, (rk, rl, tk) in enumerate(zip(row_keys, row_labels, true_keys, strict=False)):
            ax = axes[row, col]
            ...
            for patch, r in zip(bp["boxes"], results, strict=False):
    ...
    out = OUT_DIR / "fig8_source_estimation.png"
    fig.savefig(out, dpi=150, bbox_inches="tight")
    print(f"Saved {out}")
    plt.close(fig)
...
def plot_figure9(results1: list, results2: list, data1: dict, data2: dict):
    ...
    fig.tight_layout(rect=[0, 0, 1, 0.96])
    out = OUT_DIR / "fig9_location_contours.png"
    fig.savefig(out, dpi=150, bbox_inches="tight")
    print(f"Saved {out}")
    plt.close(fig)
...
def main():
    ...
    for src_num, data in [(1, data1), (2, data2)]:
        ...
        for i, (label, scheme, cls, estimated) in enumerate(MODELS):
            key_i = jax.random.fold_in(S5_KEY, (src_num - 1) * 100 + i)
